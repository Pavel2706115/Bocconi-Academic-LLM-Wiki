...
Although it is assumed that a linear relationship exists between two variables at the population level, the **parameters** β0(and) β1 **( in the regression model )**(are ) **(unknown)**(. ) **Sample data are therefore used to** ^cu5dji
...
**Please note:** b0 **cannot be interpreted as 'time spent on chat by subjects aged 0'. There is no information on subjects aged less than 20 (no predictions should be obtained corresponding to values outside the range of** X **).**

**For each additional year of age, based on the regression line, time spent on chat is 'expected' to increase on average by** b1 **= 14.64 minutes (approximately, rounded coefficient)**
...
To derive a **relative** measure (with a clear and well-defined range of variation), we introduce: - The **Total Sum of Squares (** SST ), the sum of the squared deviations of the observed values y1, y2, … , yn from their mean, \bar{y}

- The sum of the **squares of the regression (** SSR ), the sum of the squared deviations of the predictions \hat{y}_{1}, \hat{y}_{2}, … , \hat{y}_{n} from the mean of the dependent variable
...
SSR =0: the regression line coincides with a line with zero slope and intercept equal to the mean of the dependent variable
...
b = s: It is worth noting that (since 1 xy /sx(2) )
...
_Y_ x+ ε _Y_ x for each i = β0 + β1 E( i) = β0 + β1 i Thus, the information on X makes it possible to explain the **expected values** of _Y_ i (but **not** the specific realisations _Y_ i ) and there are no other factors that systematically influence E( _Y_ i) _Y x_ E( 3) β0 + β1 3 **The expected value of** _**Y**_ **varies** X **. linearly with** _Y_ x E( 2) β0 +β1 2 **It is assumed that there are no other factors that systematically influence** _**Y**_ **i.e. that the** E _Y_ β0 +β1 x1 ( 1) **differences in the expected values of** _**Y**_ **are only due to** X **(or that** X **is sufficient to explain them)** _**x x x**_ **1 2 3** ^fr8i3b
*(See also: [[Lecture 22-24_Simple Linear Regression with FULL NOTES#^8x52f9]])*

## Weak assumptions

The error term is a r.v. with constant variance, σε(2) whatever _xi_ ( **homoskedasticity** ): Var εi = σε(2) for every i = _Y_ x+ ε _Y_ σ for each i = β0 + β1 Var( i) ε(2)

_**Y x**_ Thus, the variance (and standard deviation) of _**i**_ does not depend on _**i**:_

_x_ β0 + β1 3 x β0 +β1 2 x β0 +β1 1

**The standard deviation (the amount of dispersion around the line) remains constant as** X **changes. Thus, although they have different expected values, the** _**Yi**_ **have the same variance (they can be considered a homogeneous sample with reference to the squared deviation from the mean)**
...
- Are **unbiased** for the population parameters E(\hat{β} 1) = β1
...
The **estimate** of the parameter σε(2) at a specific sample realisation y1, … , yn is:

n − b +b x SSE (yi ( 0 1 i))(2) s = ε(2) = \sum (n−2) (n−2)(= MSE) i=1

also called the **mean of the squares of the errors** ( MSE ), given by the **sum of the squares of the errors (** SSE ) divided by (n−2)

Thus, the deviations of the yi observed around the estimated line are considered estimates of the model errors, and the SSE is used to estimate the dispersion.

The estimate of - is - SSE s = ε (n−2)
...
It is not possible to assess the reliability of the estimate but only of the **estimator.** The observed estimate is just one of the possible realisations of the estimator. Remember that n= 96 , sx(2) = 88.06 , sy(2) = 30237.95 , and R(2) = 0.6241 . Therefore: 2 = SSE= (1 −R(2) ) ∙SST= (1 −R(2) ) ∙(n−1)sy 1 −0.6241 ∙95 ∙30237.95 = 1079812.31 s ε(2) = SSE/(n−2) = 1079812.314/94 = 11487.37 and: The (estimate of the) expected deviation of 2 sε 11487.37 a generic estimate of the slope from the se = \hat{β}_{1} 2(=) 95 ∙88.06(= 1.1718) parameter β11 is **1.1718** (n−1)sx

The (estimate of the) expected deviation of a generic estimate of the slope from the parameter β11 is **1.1718**

* For the commands to obtain results using R see the script

## Inference on the model’s parameters in the population

To apply more advanced inferential techniques, e.g. constructing confidence intervals or **determine the distribution of**\hat{\beta}_0 **.** testing hypotheses, it is necessary to β0 **and** β\hat{\beta}_0 1 For this purpose - at least when the sample size is not very large - it is necessary to make a further assumption on the **distribution of model errors** , in addition to the weak ones already assumed.

**Weak assumptions::** E εi = 0 and Var εi = σε(2) f or each i ; Cor(εi, εℎ) = 0 f or each i, ℎ ~ **+ The error** - **has normal distribution:** ε **Strong assumptions: Weak assumptions** i N(0, σε(2) )
...
where **`object`** is the output of the function **`lm()`** and **`which`** indicates the type of graph to be obtained. We limit attention to some of these plots (see later for others): **`which=1`** scatterplot: relationship between residuals and predictions of the dependent variable ^rdw7sc
